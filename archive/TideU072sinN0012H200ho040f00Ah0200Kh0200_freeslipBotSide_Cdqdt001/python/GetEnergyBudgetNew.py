...
nt = len(dss['time'])
energy = xr.Dataset(
                    coords={'record': dss['record'].values, 'k': dss['k']},
                    )

# get the body forces....
tidefreq = 0.0001405
print(dss.time.values[0])
print(dss)
dss['times'] = dss['time'].astype('float') / 1e9
dss['tideforce'] = -tideamp*(tidefreq**2 - f0**2) / tidefreq * np.sin(tidefreq * dss['times'])
print(dss['UVEL'], dss['tideforce'], dss['rAw'])
energy['BfTide'] = (dss['tideforce'] * dss['UVEL'] * dss['hFacW'] * dss['rAw']).sum(dim=('j', 'i_g'))
energy['Umean'] = (dss['UVEL'] * dss['hFacW'] * dss['rAw']).sum(dim=('j', 'i_g'))
energy['Vmean'] = (dss['VVEL'] * dss['hFacS'] * dss['rAs']).sum(dim=('j_g', 'i'))
energy['BfMean'] = (f0 * U0 * dss['VVEL'] * dss['hFacS'] * dss['rAs']).sum(dim=('j_g', 'i'))
energy['Z'] = dss['Z']
energy['time'] = dss['time']
energy['drF'] = dss['drF']
energy['hFacW'] = dss['hFacW']
energy['Area'] = dss['rA'].sum(dim=('j', 'i'))

energy.to_netcdf('../reduceddata/BodyForces%s.nc' % pre)
