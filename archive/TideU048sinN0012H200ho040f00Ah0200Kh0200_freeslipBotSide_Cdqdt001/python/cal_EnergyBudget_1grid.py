...
for ax, i in zip(axes, xind): 
    ...
    Fbc=uPbc+uEbc
    hdFbc=Fbc.differentiate("XC").isel(XC=i)
   
    dtEbc=np.gradient(Ebc.values,dt,axis=0)
    ddtEbc = xr.DataArray(dtEbc, coords=[time], dims=['time'])
 
    res=Conv.values-hdFbc.values-dtEbc
    Res = xr.DataArray(res, coords=[time], dims=['time'])
...
plt.show()
#plt.savefig('./figs/EnergyBudget_t_1grid.png')
#plt.tight_layout()
    #plt.show()
